...
if [ "$1" != "--skip-ui-gen" ]; then
    ...
    if [ -f "/home/flex/uis/icons/icons.qrc" ]; then
        echo "  Kompiliere icons.qrc..."
        pyrcc5 /home/flex/uis/icons/icons.qrc -o /home/flex/uis/src/icons_rc.py
    fi
    
    # Generiere alle .ui Dateien in einer Schleife
    for ui_file in /home/flex/uis/ui/*.ui; do
        if [ -f "$ui_file" ]; then
            # Extrahiere Dateinamen ohne Pfad und Erweiterung
            base_name=$(basename "$ui_file" .ui)
            output_file="/home/flex/uis/src/${base_name}.py"
...
cd /home/flex/uis/src
DISPLAY=:0 XAUTHORITY=/home/flex/.Xauthority XDG_RUNTIME_DIR=/run/user/1000 /usr/bin/python3 /home/flex/uis/src/main.py
